package com.legacycombo.activity.live;
...
import com.legacycombo.activity.catchup.GuideNewActivity;
import com.legacycombo.activity.home.WebViewActivity;
import com.legacycombo.adapter.MainListAdapter;
import com.legacycombo.apps.Constants;
import com.legacycombo.apps.MyApp;
import com.legacycombo.dialog.PackageDlg;
import com.legacycombo.dialog.PinDlg;
import com.legacycombo.dialog.SearchDlg;
import com.legacycombo.ijklib.widget.media.AndroidMediaController;
import com.legacycombo.ijklib.widget.media.IjkVideoView;
import com.legacycombo.listner.SimpleGestureFilter;
...
import static com.legacycombo.apps.Constants.setChannel_pos;
                            ...
                            case 0:
                                if (full_datas.get(category_pos).getChannels().get(channel_pos).is_favorite()) {
                                    ...
                                    for (int i = 0; i < full_datas.get(1).getChannels().size(); i++) {
                                        if (full_datas.get(1).getChannels().get(i).getName().equals(full_datas.get(category_pos).getChannels().get(channel_pos).getName())) {
                                            is_exist = true;
                                            pp = i;
                                        }
                                    }
...
//                        new Thread(()->getEpg()).start();
    ...
    private void playChannel(){
        ...
        if(channels.get(channel_pos).getStream_icon()!=null && !channels.get(channel_pos).getStream_icon().isEmpty()){
            Picasso.with(PreviewChannelIJKActivity.this).load(channels.get(channel_pos).getStream_icon())
                    .into(channel_logo);
            channel_logo.setVisibility(View.VISIBLE);
        }else {
            channel_logo.setVisibility(View.GONE);
